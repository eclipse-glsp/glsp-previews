...
`;
  ...
  var CLIENT_SESSION_ID = "smoke-" + Math.random().toString(36).slice(2, 9);
  ...
  var DIRTY_RENDER_DEBOUNCE_MS = 80;
  var dom = {
  ...
  function logEntry(kind, summary, payload) {
    ...
    sum.textContent = `[${(/* @__PURE__ */ new Date()).toLocaleTimeString()}] ${summary}`;
  ...
  var GlspClient = class {
    /** @param {Worker} worker */
    constructor(worker) {
      const reader = new import_browser.BrowserMessageReader(worker);
      const writer = new import_browser.BrowserMessageWriter(worker);
      this.connection = (0, import_browser.createMessageConnection)(reader, writer);
  ...
  function buildComputedBoundsAction(requestAction) {
    ...
    return {
      ...
      revision: (_a = requestAction.newRoot) == null ? void 0 : _a.revision,
  ...
  async function mcpFetch(extraHeaders, jsonBody, summary, options = {}) {
    ...
    const method = (_a = options.method) != null ? _a : "POST";
  ...
  function pickRenderPayload(body) {
    ...
    if (!result || typeof result !== "object") return result != null ? result : body;
  ...
  function renderObjectArrayTable(items) {
    ...
    for (const item of items) {
      ...
      for (const key of keys) {
        ...
        td.textContent = value === void 0 ? "" : typeof value === "string" ? value : JSON.stringify(value);
        row.appendChild(td);
      }
  ...
  function showInfoBar() {
    ...
    (_a = document.getElementById("server-messages-section")) == null ? void 0 : _a.classList.remove("section-hidden");
  }
  function renderStatus(action) {
    var _a, _b;
    const severity = ((_a = action.severity) != null ? _a : "info").toLowerCase();
    const message = ((_b = action.message) != null ? _b : "").trim();
  ...
  function startProgress(action) {
    var _a;
    dom.progressTitle.textContent = (_a = action.title) != null ? _a : "In progress\u2026";
  ...
  function clipLineToShape(shape, fromX, fromY) {
    if (shape.kind === "diamond") {
      const dx2 = fromX - shape.cx;
      const dy2 = fromY - shape.cy;
      if (dx2 === 0 && dy2 === 0) return { x: shape.cx, y: shape.cy };
      const r = 1 / (Math.abs(dx2) / shape.halfW + Math.abs(dy2) / shape.halfH);
      return { x: shape.cx + dx2 * r, y: shape.cy + dy2 * r };
    }
  ...
  function partitionElements(elements) {
    ...
    for (const element of elements) {
      const type = (_a = element.type) != null ? _a : "";
  ...
  function computeViewBox(nodes) {
    ...
    for (const node of nodes) {
      ...
      const { width: width2, height: height2 } = node.size;
      if (x < minX) minX = x;
      if (y < minY) minY = y;
      if (x + width2 > maxX) maxX = x + width2;
      if (y + height2 > maxY) maxY = y + height2;
    }
    const padding = 20;
  ...
  async function fetchAndRenderDiagram() {
    ...
    const elements = (_c = (_b = (_a = result.body) == null ? void 0 : _a.result) == null ? void 0 : _b.structuredContent) == null ? void 0 : _c.elements;
  ...
  async function boot() {
    ...
    worker.addEventListener("error", (event) => {
      var _a2;
      return logEntry("error", "Worker error", (_a2 = event.message) != null ? _a2 : String(event));
    });
    try {
      ...
      dom.bootStatus.textContent = `Boot failed \u2014 ${(_a = err.message) != null ? _a : err}`;
  ...
  dom.btnMcpInit.addEventListener("click", async () => {
    ...
    mcpSessionId = (_a = result.headers["mcp-session-id"]) != null ? _a : "";
    if (!mcpSessionId) {
  ...
  dom.btnMcpUndo.addEventListener("click", async () => {
    ...
    await mcpToolCall("undo", { sessionId: CLIENT_SESSION_ID, commandsToUndo: count }, `(undo \xB7 ${count})`);
    undoStack.pop();
  ...
  dom.btnMcpRedo.addEventListener("click", async () => {
    ...
    await mcpToolCall("redo", { sessionId: CLIENT_SESSION_ID, commandsToRedo: count }, `(redo \xB7 ${count})`);
    redoStack.pop();
  ...
  function trackDispatched(toolResult) {
    var _a, _b, _c;
    const count = (_c = (_b = (_a = toolResult == null ? void 0 : toolResult.body) == null ? void 0 : _a.result) == null ? void 0 : _b.structuredContent) == null ? void 0 : _c.dispatchedCommands;
  ...
  function rememberCreatedTaskIds(toolResult) {
    var _a, _b, _c;
    const created = (_c = (_b = (_a = toolResult == null ? void 0 : toolResult.body) == null ? void 0 : _a.result) == null ? void 0 : _b.structuredContent) == null ? void 0 : _c.createdNodes;
  ...
  dom.btnMcpTerminate.addEventListener("click", async () => {
    ...
    await mcpFetch(mcpHeaders(), void 0, "(terminate)", { method: "DELETE" });
    mcpSessionId = "";
